...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
    ...
    public List<CompilationUnit> getCompilationUnits() {
       ...
       /* if("Y".compareToIgnoreCase(ExecuteCreateModel.properties.getProperty("makeRepository"))==0) {
            TopLevelClass topLevelClass = new TopLevelClass(type);
            topLevelClass.setVisibility(JavaVisibility.PUBLIC);
            commentGenerator.addJavaFileComment(topLevelClass);
            createGetNameSpace(topLevelClass);
            createDaoResource(topLevelClass);
            topLevelClass.setSuperClass("BaseRepository");
            topLevelClass.addAnnotation("@Repository");
            topLevelClass.addImportedType("com.dcits.ensemble.dao.BaseRepository");
            topLevelClass.addImportedType("org.springframework.stereotype.Repository");

            answer.add(topLevelClass);
        }*/
        //增加Dao层生成文件是否为class分支
        if("Y".compareToIgnoreCase(ExecuteCreateModel.properties.getProperty(CompsName.DaoClassType))==0){
            TopLevelClass topLevelClass = new TopLevelClass(type);
            topLevelClass.setVisibility(JavaVisibility.PUBLIC);
            commentGenerator.addJavaFileComment(topLevelClass);
            ...
            createGetNameSpace(topLevelClass);
            createPropertyDefalutNameSpace(topLevelClass);
            topLevelClass.addAnnotation("@Repository");
            topLevelClass.addImportedType(ExecuteCreateModel.properties.getProperty(CompsName.DBDaoRootClassFullPath));
            topLevelClass.addImportedType("org.springframework.stereotype.Repository");
    ...
    /*private void createDaoResource(TopLevelClass topLevelClass) {
        String daoInterfaceType = introspectedTable.getDAOInterfaceType();
        FullyQualifiedTable fullyQualifiedTable = introspectedTable.getFullyQualifiedTable();
        Field field = new Field();
        field.setVisibility(JavaVisibility.PRIVATE);
        field.setType(fullyQualifiedTable);
        field.setName("DEFAULT_NAME_SPACE");
        field.setInitializationString("\""+daoInterfaceType+"\"");
        topLevelClass.addField(field);
    }*/

    private void createPropertyDefalutNameSpace(TopLevelClass topLevelClass) {
        String daoInterfaceType = introspectedTable.getDAOInterfaceType();
        //自动生成Dao为全大写，在此更改为小写
        daoInterfaceType=daoInterfaceType.replaceAll("DAO","Dao");
        Field field = new Field();
    ...
    private void createGetNameSpace(TopLevelClass topLevelClass) {
